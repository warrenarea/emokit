...
def handler(self, data):
    assert data[0] == 0
    packet = decrypt((''.join(map(chr, data[1:]))), headset_config)
    yield Packet
    self.packets_received += 1
    return True


def setup_crypto(headset, is_research=True):
    '''
    `is_research` should be True if EPOC+, try False if you have an EPOC
    '''
    if system_platform == "Windows":
        
        setup_windows(headset)
    else:
        hidapi.hid_init()
...
def get_packets():
    try:
        if system_platform == "Windows":
            setup_windows(headset_config)
        else:
            setup_crypto(headset_config)
        ...
        if system_platform == "Windows":
            hid.headset_config['device'].close()
        else:
